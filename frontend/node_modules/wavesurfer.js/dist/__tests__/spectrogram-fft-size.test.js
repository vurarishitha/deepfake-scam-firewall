...
function runWorker(signal, options) {
    ...
    self.onmessage({
        data: {
            ...
            id: 'test',
            audioData: [signal],
            options: Object.assign({ startTime: 0, endTime: signal.length / SAMPLE_RATE, sampleRate: SAMPLE_RATE, windowFunc: 'hann', scale: 'linear', gainDB: 20, rangeDB: 80, splitChannels: false }, options),
        },
    });
...
describe.each([
...
])('%s fftSize option validation', (_name, Plugin) => {
    ...
    it('rejects large non-powers-of-two that fool 32-bit bitwise checks', () => {
        expect(() => Plugin.create({ fftSize: Math.pow(2, 32) + 1 })).toThrow(TypeError);
    });
...
describe('worker compute with fftSize', () => {
    ...
    it('renders non-linear scales with a padded FFT (no all-zero output)', () => {
        ...
        expect(result[0].some((frame) => frame.some((value) => value > 0))).toBe(true);
    });
...
describe('main-thread compute with fftSize', () => {
    it('matches the worker output byte for byte on the default (no worker) path', () => __awaiter(void 0, void 0, void 0, function* () {
        const signal = makeSine(4000);
        const plugin = Spectrogram.create({ fftSamples: 64, fftSize: 512, noverlap: 32, scale: 'mel' });
        const buffer = {
        ...
        const mainResult = yield plugin.getFrequencies(buffer);
        ...
        expect(mainResult[0].some((frame) => frame.some((value) => value > 0))).toBe(true);
