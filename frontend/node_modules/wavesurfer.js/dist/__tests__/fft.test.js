import FFT, { applyFilterBank, applySparseFilterBank, createFilterBankForScale, createSparseFilterBankForScale, magnitudesToColorIndices, magnitudesToDb, dbToColorIndices, createPreEmphasisTilt, getBinFrequencies, SILENCE_FLOOR_DB, } from '../fft.js';
...
describe('magnitudesToColorIndices', () => {
    ...
    function legacyLoop(spectrum, gainDB, rangeDB) {
    ...
    const dbToMagnitude = (db) => Math.pow(10, db / 20);
    it('is byte-identical to the reference loop, including boundary values', () => {
        ...
        const values = [0, 1e-15, 1e-12, 2e-12, 1];
        ...
        expect(Array.from(magnitudesToColorIndices(spectrum, -gainDB, rangeDB))).toEqual(Array.from(legacyLoop(spectrum, gainDB, rangeDB)));
    });
    it('is byte-identical to the reference loop across gain/range combinations on a real spectrum', () => {
        ...
        const spectrum = new FFT(512, 16000, 'hann', undefined).calculateSpectrum(signal);
...
describe('preEmphasis tilt and autoGain helpers', () => {
    ...
    it('honors an explicit blackman alpha of 0 instead of coercing it to the default', () => {
        const explicitZero = new FFT(64, 8000, 'blackman', 0);
        const defaulted = new FFT(64, 8000, 'blackman', undefined);
