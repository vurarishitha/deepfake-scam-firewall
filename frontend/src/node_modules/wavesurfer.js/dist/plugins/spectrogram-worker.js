...
import FFT, { createSparseFilterBankForScale, applySparseFilterBank, magnitudesToColorIndices, magnitudesToDb, dbToColorIndices, createPreEmphasisTilt, getBinFrequencies, SILENCE_FLOOR_DB, AUTO_GAIN_BUFFER_BUDGET_BYTES, } from '../fft.js';
...
function calculateFrequencies(audioChannels, options) {
    const { startTime, endTime, sampleRate, fftSamples, fftSize, windowFunc, alpha, noverlap, scale, gainDB, rangeDB, preEmphasis, autoGain, autoGainBufferBudgetBytes, splitChannels, } = options;
    ...
    const tilt = preEmphasis
        ...
        : null;
    const computeSpectrum = (channelData, sample) => {
    ...
    const budgetBytes = autoGainBufferBudgetBytes !== null && autoGainBufferBudgetBytes !== void 0 ? autoGainBufferBudgetBytes : AUTO_GAIN_BUFFER_BUDGET_BYTES;
    let maxDb = -Infinity;
    if (estimatedBytes < budgetBytes) {
        ...
        for (const channelDb of dbFrames) {
            frequencies.push(channelDb.map((db) => (silent ? new Uint8Array(db.length) : dbToColorIndices(db, maxDb, rangeDB))));
        }
        return frequencies;
    }
